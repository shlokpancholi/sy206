...
#recursive print tree method and helper for printing out the tree and the associated N codes and job descriptions
    ...
    def search(self, n_code):
        return self._search_recursive(self.root, n_code)
    def _search_recursive(self, current_node, n_code):
        if current_node is None or current_node.n_code == n_code:
            return current_node
        if n_code < current_node.n_code:
            return self._search_recursive(current_node.left, n_code)
        return self._search_recursive(current_node.right, n_code)
#recursive search function that works with a menu I made for users to find out what billet an N-Code corresponds to
...
def main():
    ...
    while True:
        choice = input("Enter 1 to search for an N-Code or 2 to exit: ")
        if choice.lower() == '1':
            n_code = input("N-Code: ")
            node = opnav_tree.search(n_code)
            if node:
                print(f"That N-Code is the: ", node.description)
            else:
                print("Invalid N-Code")
        elif choice.lower() == '2':
            print("Exiting...")
            break
        else:
            print("Error...Try Again")
#utilized a while true implementation of a user menu that I used all the time in SY201 and once or twice in SY206 so far
